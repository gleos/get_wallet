...
for i in range(1,20):
    ...
    creds = ServiceAccountCredentials.from_json_keyfile_name('erudite-bonbon-340016-c17d1f364404.json', scope)
...
# scope = ['https://spreadsheets.google.com/feeds','https://www.googleapis.com/auth/drive']
# # add credentials to the account
# creds = ServiceAccountCredentials.from_json_keyfile_name('/Users/gregoire/Downloads/erudite-bonbon-340016-c17d1f364404.json', scope)
# # authorize the clientsheet 
# client = gspread.authorize(creds)
# sheet = client.open('wallets_list')
# sheet_instance = sheet.get_worksheet(0)
...
# solana_client = Client(RPC)
# result  = solana_client.get_signatures_for_address(casino2)
# print('len:', len(result['result']))
# transactions = []
# # I use `[:5]` to display only first 5 values
# for number, item in enumerate(result['result'], 1):
...
# a =solana_client.get_confirmed_transaction(transactions[1])
# print(a)
# b = a['result']['transaction']['message']['accountKeys']
# print(b)

# for i in b:
#     a = solana_client.get_balance(i)
#     if a != 2268960:
#         print(a)
